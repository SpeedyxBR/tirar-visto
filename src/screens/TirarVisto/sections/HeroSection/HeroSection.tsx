    ...
    <section className="relative w-full h-[940px] bg-white overflow-hidden">
        ...
        <div className="flex items-center gap-1">
          {navItems.map((item, index) => (
            <Button
              key={index}
              variant="ghost"
              className={`h-11 px-3 py-2.5 ${item.isBold ? "font-bold" : "font-normal"} text-[#222223] text-base`}
            >
              {item.title}
            </Button>
          ))}
      ...
      <div className="flex items-center gap-7 absolute bottom-[60px] left-[60px]">
        {statistics.map((stat, index) => (
          <div
            key={index}
            className={`flex flex-col w-40 h-[100px] items-start gap-2 ${index < statistics.length - 1 ? "border-r border-neutral-200" : ""}`}
          >
